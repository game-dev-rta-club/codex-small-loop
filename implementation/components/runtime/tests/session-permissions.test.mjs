...
function context() {
  return { cwd: "/project", workspace_roots: ["/project"], model: "gpt-5.6-sol", effort: "medium", approval_policy: "never",
    ...
    permission_profile: { type: "managed", network: "restricted", file_system: { type: "restricted", entries: [
      ...
      ...[".git", ".agents", ".codex"].map((name) => ({ path: { type: "path", path: `/project/${name}` }, access: "read", missing_path_behavior: "skip" })),
    ] } } };
}
function read(...contexts) {
  return readCodexTaskRunSettings(Readable.from([
    JSON.stringify({ type: "session_meta", payload: { id: "task", cwd: "/project" } }),
...
test("custom read-only exceptions are rejected instead of granting workspace-wide writes", async () => {
  const value = context();
  value.permission_profile.file_system.entries.push({ path: { type: "path", path: "/project/private" }, access: "read" });
