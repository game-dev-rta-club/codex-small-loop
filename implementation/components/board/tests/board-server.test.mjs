...
test("Sonner opens indexed files and Work directories and propagates opener revalidation failures", async (t) => {
  ...
  const indexedProjection = { version: 13, workGraph: { status: "missing" }, files: { root: { path: ".", name: ".", type: "directory",
...
test("Sonner shares project single-flight and the exact response-byte bound", async (t) => {
  ...
  t.after(() => rm(projectRoot, { recursive: true, force: true }));
  const projection = { version: 13, workGraph: { status: "missing" }, files: { root: { path: ".", name: ".", type: "directory", children: [] } }, runtime: { status: "missing" } };
