...
test("Activity Bar switches Activity and Sonner while Work Graph, Files, and Runtime remain accessible", async () => {
  ...
  assert.match(js, /for \(const \{ extension, count \} of node\.counts\)/);
  assert.match(js, /line\.className = "file-count-line"/);
  assert.match(js, /line\.textContent = `\$\{count\} \$\{extension \?\? "extensionless"\}`/);
  assert.doesNotMatch(js, /∑/);
  assert.match(css, /\.file-count-line \{ display: block; \}/);
