# Welcome to Codex Small Loop

Use AI with Loop Engineering.

![Codex Small Loop hero image]({{WELCOME_IMAGE_ABSOLUTE_PATH}})

### Recommended Tools

The following tools make development easier.

| Tool | What it does | Setup |
|---|---|---|
| **Slack Plugin** | Receive progress and completion notifications | [Add Plugin →](plugin://slack@openai-curated-remote) |
| **Obsidian App** | Read project documentation | [Open Download Page →](https://obsidian.md/download) |

---

### Next Action

Let's choose the Agent model.

| Model | AA Score | AA Cost | AA Time | Recommended Plan | Best For |
|---|---:|---:|---:|---|---|
| Luna Max | 52 | US$172 | 108 sec | Plus | Keeping costs low |
| **Terra Medium** | 47 | US$192 | 37 sec | Plus | Finishing quickly |
| Sol Low | 51 | US$344 | 46 sec | Pro 5x | Balancing cost, speed, and accuracy |
| Sol Medium | 56 | US$580 | 79 sec | Pro 20x | Prioritizing accuracy |

| Speed | Execution Speed | Token Usage |
|---|---:|---:|
| **1x** | 1x | 1x |
| **1.5x** | 1.5x faster | 2.5x |

**May I start with the standard Terra Medium model? Unless you explicitly select 1.5x, execution remains at 1x.**
